...
| V1 | code agents (coordinator routes per CLAUDE.md: `react-specialist` for `Chat.jsx`/client, `websocket-engineer` for transport, `backend-developer` for the server, `llm-architect` for the single-DM-trigger) | implementation | **DONE — autonomous overnight build 2026-05-26 (user-directed, unattended). All Phases 0–7 implemented, each test-green and committed on `feature/multiplayer`; full suite 405 pass / 2 skip (no regressions). Merged to `master` and pushed.** | **G1 cleared** + D2 phased build plan |
...
| V1.P5 | `react-specialist` + `websocket-engineer` | `src/lib/turnStateMachine.js`(+test), `server/sync-server.mjs`, `src/components/Chat.jsx`, `src/components/PartyStrip.jsx`(+test), `src/components/HistoryPanel.jsx`(+test) | **DONE** — pure `phaseReducer`/`isActiveTurn` shared client+server; server `NOT_YOUR_TURN` gate uses connection-bound displayName (sec C, per-message name ignored); client input gating (`myTurn`) + "Waiting for {name}…" + combat HUD (PartyStrip dim, HistoryPanel turn section). Single-player unchanged. 392 pass/12 skip. | P4 |
| V1.P6 | `websocket-engineer` | `server/sync-server.mjs`, `src/components/Chat.jsx` | **DONE** — disconnect keeps entry as `disconnected` in presence:update; DM stream survives triggerer disconnect (broadcast skips CLOSED, .md persists); no crash/deadlock on active-combat-player disconnect; rejoin clears stale CLOSED entry + full session:state; injectable `roomGcMs` orphaned-room GC (cancelled on activity); client presence status dots. 397 pass/7 skip. | P5 |
| V1.P7 | `backend-developer` + `react-specialist` | `server/sync-server.mjs`, `server/sync-server.multiplayer.test.mjs`, `src/hooks/useSessionPersistence.test.jsx` | **DONE** — HTTP PUT now passes v2 fields through `serializeSession` (MC-3 server side; sessionId still path-derived; 409 LWW intact); v1-shaped PUT → v2 defaults on GET; v1/v2 `.md` load + single-player + WS-path stale-block all covered. 405 pass/2 skip (only the non-gate `useWebSocket` M7 placeholder remains). | P6 |
